...
%   of a permutation test based on Pearson's r. If X and Y are matrices,
%   multiple permutation tests are performed simultaneously between each
%   corresponding pair of columns in X and Y and family-wise error rate
%   (FWER) is controlled using the max statistic correction method (Blair
%   et al., 1994). This method provides strong control of FWER, even for
%   small sample sizes, and is much more powerful than traditional
%   correction methods (Groppe et al., 2011). For nonlinear correlations,
%   the raw data may be transformed to rank orders using a Spearman's or a
%   rankit transformation (Bishara & Hittner, 2012). If Y is not entered,
%   the correlation between each pair of columns in X is computed and
%   output as a correlation matrix.
%
...
%                      and for Spearman rank correlations, Fieller et al.'s
%                      estimate is used (Bishara & Hittner, 2017).
%       'rcrit' 	-- critical r-value(s) for the given alpha level
...
if nargout > 2
    ...
    clear p ci rcrit estal ciLwr ciUpr
