...
for i = groups
    xg = x(:,group==i);
    if nanflag
        n(i) = sum(nansum(~isnan(xg)));
        means(i) = sum(nansum(xg))/n(i);
        ess = ess + sum(nansum((xg-means(i)).^2));
        rss = rss + n(i)*sum(nansum((means(i)-gm).^2));
    else
        n(i) = sum(sum(~isnan(xg)));
        means(i) = sum(sum(xg))/n(i);
        ess = ess + sum(sum((xg-means(i)).^2));
        rss = rss + n(i)*sum(sum((means(i)-gm).^2));
    end
end
    ...
    for i = 1:arg.nperm
        ...
        for j = groups
            ...
            if nanflag
                np = nansum(~isnan(xg));
                mp = nansum(xg)/np;
                essp = essp + nansum((xg-mp).^2);
                rssp = rssp + np*nansum((mp-gm).^2);
            else
                np = sum(sum(~isnan(xg)));
                mp = sum(sum(xg))/np;
                essp = essp + sum(sum((xg-mp).^2));
                rssp = rssp + np*sum(sum((mp-gm).^2));
            end
        end
