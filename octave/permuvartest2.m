...
if nanflag
    varx = (nansum(x.^2)-(nansum(x).^2)./nobsx)./df1;
    vary = (nansum(y.^2)-(nansum(y).^2)./nobsy)./df2;
else
    varx = (sum(x.^2)-(sum(x).^2)./nobsx)./df1;
    vary = (sum(y.^2)-(sum(y).^2)./nobsy)./df2;
end
...
if nargout > 1
    ...
    for i = 1:arg.nperm
        ...
        x2 = x(i2(:,i),:);
        if nanflag
            var1 = (nansum(x1.^2)-(nansum(x1).^2)./nobsx)./df1;
            var2 = (nansum(x2.^2)-(nansum(x2).^2)./nobsy)./df2;
        else
