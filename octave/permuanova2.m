...
if nanflag
    ...
    % Compute grand mean
    gm = sum(nansum(xm))/nobs;

    % Compute column sum of squares
    colmeans = mean(xm,1,'omitnan');
    css = coln*sum(nansum((colmeans-gm).^2));

    % Compute row sum of squares
    rowmeans = mean(xm,2,'omitnan');
    rss = rown*sum(nansum((rowmeans-gm).^2));

    % Compute interaction sum of squares
    factor = reps*cols*grows*gm^2;
    iss = reps*sum(nansum(xm.^2))-css-rss-factor;

    % Compute total sum of squares
    tss = sum(nansum(x.^2))-factor;
    ...
    % Compute column sum of squares
    colmeans = mean(xm,1);
    css = coln*sum(sum((colmeans-gm).^2));

    % Compute row sum of squares
    rowmeans = mean(xm,2);
    rss = rown*sum(sum((rowmeans-gm).^2));

    % Compute interaction sum of squares
    factor = reps*cols*grows*gm^2;
    iss = reps*sum(sum(xm.^2))-css-rss-factor;

    % Compute total sum of squares
    tss = sum(sum(x.^2))-factor;
...
if nargout > 1
    ...
    for i = 1:arg.nperm
        ...
        if nanflag
            colmeansp = mean(xm,1,'omitnan');
            cssp = coln*sum(nansum((colmeansp-gm).^2));
            rowmeansp = mean(xm,2,'omitnan');
            rssp = rown*sum(nansum((rowmeansp-gm).^2));
            issp = reps*sum(nansum(xm.^2))-cssp-rssp-factor;
        else
            colmeansp = mean(xm,1);
            cssp = coln*sum(sum((colmeansp-gm).^2));
            rowmeansp = mean(xm,2);
            rssp = rown*sum(sum((rowmeansp-gm).^2));
            issp = reps*sum(sum(xm.^2))-cssp-rssp-factor;
        end
        if reps > 1
            if nanflag
                tssp = sum(nansum(xp.^2))-factor;
            else
                tssp = sum(sum(xp.^2))-factor;
            end
