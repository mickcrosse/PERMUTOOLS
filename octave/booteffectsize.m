...
if arg.paired
    ...
    switch arg.effect
        case 'mediandiff'
            if nanflag
                mu = median(diffxy,'omitnan');
            else
                mu = median(diffxy);
            end
        otherwise
            if nanflag
                mu = nansum(diffxy)./nobs;
            else
                mu = sum(diffxy)./nobs;
            end
    end
    ...
    switch arg.effect
        ...
        case 'mediandiff'
            if nanflag
                mu = median(x,'omitnan')-median(y,'omitnan');
            else
                mu = median(x)-median(y);
            end
        otherwise
...
if nargout > 1
    ...
    for i = 1:nvar
        ...
        if nanflag
            smxb = nansum(xb);
            smyb = nansum(yb);
            varxb = (nansum(xb.^2)-(smxb.^2)/nobsx(i))/dfx(i);
            varyb = (nansum(yb.^2)-(smyb.^2)/nobsy(i))/dfy(i);
        else
            smxb = sum(xb);
            smyb = sum(yb);
            varxb = (sum(xb.^2)-(smxb.^2)/nobsx(i))/dfx(i);
            varyb = (sum(yb.^2)-(smyb.^2)/nobsy(i))/dfy(i);
        end
        if arg.paired
            switch arg.effect
                case 'cliff'
            ...
            switch arg.effect
                case 'mediandiff'
                    if nanflag
                        mub = median(diffxyb,'omitnan');
                    else
                        mub = median(diffxyb);
                    end
                otherwise
                    if nanflag
                        mub = nansum(diffxyb)/nobs(i);
                    else
                        mub = sum(diffxyb)/nobs(i);
                    end
            end
            ...
            switch arg.effect
                ...
                case 'mediandiff'
                    if nanflag
                        mub = median(xb,'omitnan')-median(yb,'omitnan');
                    else
                        mub = median(xb)-median(yb);
                    end
                otherwise
